...
machine, mirroring MockDataPipelineService exactly.

RealSnowflakeLoadService executes
    COPY INTO "DB"."SCHEMA"."TABLE" FROM 's3://...'
    STORAGE_INTEGRATION = <name> FILE_FORMAT = (TYPE = PARQUET)
    MATCH_BY_COLUMN_NAME = CASE_INSENSITIVE
asynchronously (execute_async -> sfqid) and polls get_query_status -- the
reverse of build_unload_sql. Column mapping is BY NAME, not position: the
destination table's columns must be named to match the scored output (the
execute_model step's preserved input features plus its prediction column).
Every run APPENDS -- there is no OVERWRITE here, unlike the unload; Snowflake
also tracks COPY INTO <table> load history per staged file path for 64 days
and will not silently reload an already-loaded file, an extra safety net on
top of each run's source prefix already being unique.

Traceability back to the run uses the SAME mechanism as the unload step:
the step's persisted `snowflakeQueryId` plus the platform's own audit log
(job -> step -> snowflakeQueryId) is the record tying a load back to the
run and the human who triggered it -- no query tagging or row-level
run-id/date columns are injected into the loaded data itself, since that
would require every destination table to reserve platform-owned columns.
...
def build_load_sql(step_config: dict) -> str:
    """The COPY INTO <table> statement for one load_to_snowflake step.
    `sourceS3Uri` is a job_service-resolved field (never author-supplied —
    it's always the run's own execute_model output). Raises ValueError on
    any identifier/URI that fails validation."""
    params = step_config.get("snowflakeParams") or {}
    ...
    # MATCH_BY_COLUMN_NAME: no SELECT/column list to maintain as scored
    # output columns evolve — Snowflake maps parquet columns onto the
    # destination table's columns by name (missing/extra columns are
    # Snowflake's default graceful behavior, not a platform decision).
    return (
    ...
    def get_status(
        ...
        return {
            ...
            "output": {
                ...
                "table": _qualified_table(step_config or {}),
            },
        }
...
class RealSnowflakeLoadService(SnowflakeLoadService):
    """Live S3 -> Snowflake load via an asynchronous COPY INTO <table>."""
    ...
    def get_status(
        ...
        try:
            ...
            output = {"table": _qualified_table(step_config or {})}
            try:
